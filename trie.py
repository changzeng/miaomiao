...
class Trie:
    def __init__(self):
        ...
        if os.path.exists("data/trie.data"):
            # read from file
            with open("data/trie.data") as fd:
    ...
    def buildTrie(self):
        ...
        with open("data/dict.txt", encoding="utf8", errors="ignore") as fd:
            line = fd.readline()
            while line is not '':
                line = line.strip()
                items = line.split(" ")
    ...
    def all_cut(self, sentence):
        ...
        visited = set()
        visited.add(0)
        dis = [float("inf") for i in range(len(sentence) + 1)]
        ...
        while True:
            ...
            for i in range(1, length + 1):
                if i not in visited and dis[i] != float("inf"):
                    if (dis[i] < min_number):
                        min_number = dis[i]
                        min_index = i
